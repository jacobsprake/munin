# Post-Quantum Cryptography: Why ML-DSA (FIPS 204) is Required
...
**Munin uses ML-DSA (FIPS 204), Dilithium3 parameter set, for all critical command signatures. This decision is not optional—it is architecturally necessary for a system that must remain secure through 2030 and beyond.**
...
**The Decision**: ML-DSA (FIPS 204) is the NIST-standardized Module-Lattice-Based Digital Signature Standard; the Dilithium3 parameter set provides the best balance of security, performance, and signature size for sovereign infrastructure systems.
...
**ML-DSA is NIST FIPS 204** (Module-Lattice-Based Digital Signature Standard). The Dilithium3 parameter set (ML-DSA-65) was selected after a 6-year competition (2016-2022) that evaluated 82 candidate algorithms.
...
|-----------|---------------|---------------|----------|-------------|
| **ML-DSA (Dilithium3)** | 128-bit (quantum) | 3,293 bytes | 1,952 bytes (public) | Fast |
| RSA-2048 | 112-bit (classical) | 256 bytes | 256 bytes (public) | Slow |
| ECDSA (P-256) | 128-bit (classical) | 64 bytes | 64 bytes (public) | Fast |

**Trade-offs:**
- **Signature Size**: ML-DSA (Dilithium3) signatures are larger (3,293 bytes vs. 256 bytes for RSA), but this is acceptable for sovereign infrastructure systems where security is paramount
- **Performance**: ML-DSA is faster than RSA for signing and verification
- **Security**: ML-DSA (Dilithium3) provides 128-bit quantum security, while RSA-2048 provides only 112-bit classical security (and will be broken by quantum computers)
...
- **ML-DSA (Dilithium)**: Used for digital signatures (authentication)

**Munin needs digital signatures, not encryption, for handshake packets. Therefore, ML-DSA (FIPS 204) is the correct choice.**
...
def generate_signature(private_key, message):
    """
    Generate ML-DSA (Dilithium3) signature for message.
    
    Args:
        private_key: ML-DSA (Dilithium3) private key (4,000 bytes)
        message: Message to sign (handshake packet)
    
    Returns:
        signature: ML-DSA (Dilithium3) signature (3,293 bytes)
    """
    # ML-DSA (FIPS 204) signing algorithm
...
def verify_signature(public_key, message, signature):
    ...
    Args:
        public_key: ML-DSA (Dilithium3) public key (1,952 bytes)
...
4. Sign hash with ML-DSA: `signature = MLDSA_SIGN(private_key, hash)`
5. Attach signature to packet

**Verification Process:**
1. Extract signature and public key from packet
2. Recompute hash: `hash = SHA-256(packet_json)`
3. Verify signature: `is_valid = MLDSA_VERIFY(public_key, hash, signature)`
...
**ML-DSA (Dilithium3) provides 128-bit quantum security**, meaning:
...
**ML-DSA also provides strong classical security:**
- Resistant to classical attacks (lattice reduction, etc.)
- No known classical attacks that break ML-DSA (Dilithium3)
- Security based on hardness of lattice problems (well-studied)

### Side-Channel Resistance

**ML-DSA is designed to be side-channel resistant:**
...
- **Algorithm Selected**: ML-DSA (FIPS 204), Dilithium3 parameter set
...
- **Phase 1 (Months 1-3)**: Implement ML-DSA (FIPS 204) in Rust
...
**ML-DSA (FIPS 204) is required for Munin.**

**The state's infrastructure must remain secure through 2030 and beyond. Classical cryptography (RSA, ECDSA) will be broken by quantum computers. Post-quantum cryptography (ML-DSA, FIPS 204) is the only solution.**
